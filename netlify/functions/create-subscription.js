...
export const handler = async event => {
  ...
  try{
    ...
    if(priceId.startsWith('prod_')){
      ...
      if(!priceId)return json(500,{error:'O produto Stripe não possui um preço padrão. Defina um preço recorrente como default_price ou use diretamente o ID price_.'});
    }
    if(!priceId?.startsWith('price_'))return json(500,{error:'STRIPE_PRO_PRICE_ID deve conter um ID price_ ou prod_ válido.'});
    const price=await stripe.prices.retrieve(priceId);
    if(!price.active||!price.recurring)return json(500,{error:'O preço Stripe precisa estar ativo e ser recorrente.'});
